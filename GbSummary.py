...
_SELECT_SNAPSHOTS = 'SELECT idsnapshot, timestamp, person_id from guestbook_personsnapshot WHERE guestbook_personsnapshot.timestamp between \'%s\' and \'%s\''
...
_SELECT_QUERIES   = ['SELECT * from guestbook_personsnapshot WHERE guestbook_personsnapshot.timestamp between \'%s\' and \'%s\'',
...
def summary(whenFromDatetime, whenToDatetime):
  con = p.connect("dbname='guestbookdb' user='gb_readonly' host='localhost' port='2345'")
  cur = con.cursor()
  serviceQuery = 'SELECT name from guestbook_service'
  cur.execute(serviceQuery)
  services = cur.fetchall()
  snapshotsQuery = _SELECT_SNAPSHOTS  % (whenFromDatetime, whenToDatetime)  
  print(snapshotsQuery)
  cur.execute(snapshotsQuery)
  snapshots = cur.fetchall()
  for snapshot in snapshots:
    snapshotid = snapshot[0]
    timestamp = snapshot[1]
    personid = snapshot[2]
    personQuery = 'SELECT aliasname, firstname, lastname from guestbook_person WHERE idPerson=%s' % (personid)
    cur.execute(personQuery)
    person  = cur.fetchall()[0]
    print('(%s %s) %s at %s:' % (person[1], person[2], person[0], timestamp))
    clientServicesQuery = 'SELECT service_id from guestbook_personservicerequest WHERE connection_id=%s' % str(snapshotid)
    cur.execute(clientServicesQuery)
    clientServices  = cur.fetchall()
    for clientService in clientServices:
      eng, esp = str(services[clientService[0]][0]).split('/')
      print('               %s' % (eng))
    
    
  
def howMany(whenFrom, whenTo):
  ...
  return summary(whenFromDatetime, whenToDatetime)
